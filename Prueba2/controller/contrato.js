    ...
    getTabla: function (callback) {
        iniciar.getConnection(function (err, connection) {
            if (err) {
                ...
                connection.query('SELECT  * FROM v_contrato', function (error, results, fields) {//
                    if (error) {
                        callback('error en la consulta: ' + error, null);
                    } else {
                        callback(null, {contratos: results});
    ...
    insert: function (datos, callback) {//la variable datos contiene todo lo que se va a insertar en formato json
        iniciar.getConnection(function (err, connection) {
            if (err) {
                ...
                connection.query("SELECT sp_contrato('"+datos.id+"','"+datos.inicio+"','"+datos.final+"','"+datos.descripcion+"','"+datos.cliente+"','"+datos.usuario+"') AS nota", datos, function (error, results, rows) {
